...
// Markers must be evidence of generation, not a mention of it. `// prettier-
// ignore-start` is a formatting directive on hand-written tables; a loader you
// wrote mentions `__webpack_require__` once; a sourcemap utility contains the
// sourceMappingURL string. Each of those had real source deleted from the
// ranking. Banners are therefore anchored to the first few lines, and bundler
// tokens must appear repeatedly.
const GENERATED_BANNER = /^\s*(?:\/\/|#|\/\*)\s*(?:@generated\b|AUTO-GENERATED|Code generated by|DO NOT EDIT\b[^\n]{0,20}$)/im;
const GENERATED_MARKERS = [
  /\/\/\s*#region\s+\0?rolldown/,
...
function looksGenerated(file, src, lineCount) {
  ...
  // A generation banner lives in the first few lines, not anywhere in 4KB.
  if (GENERATED_BANNER.test(src.split('\n').slice(0, 4).join('\n'))) return 'generated';
  for (const re of GENERATED_MARKERS) if (re.test(head)) return 'generated';
  // Bundler runtimes reference these constantly; source mentions them once.
  if ((src.match(/__webpack_require__/g) || []).length > 3) return 'generated';
  if (/sourceMappingURL=data:application\/json;base64/.test(src.slice(-2000))) return 'generated';
...
function stripComments(src) {
  ...
  for (let i = 0; i < src.length;) {
    ...
    // ...and a regex literal never starts with a space or `=`. Division almost
    // always has one (`total! / count`, `a++ / 2`), which is what made those
    // read as regex openers and swallow the rest of the line.
    if (ch === '/' && src[i + 1] !== ' ' && src[i + 1] !== '=' && /(?:[([{,;:=!&|?+\-*%~^]|\breturn|\btypeof|\bcase)\s*$/.test(out)) {
    ...
    if (ch === '`') {
      ...
      // Blank only the literal TEXT of a multi-line template; `${...}` holds real
      // code, and blanking it deleted genuine `await import('./x')` edges.
      out += body.includes('\n')
        ? body.replace(/\$\{[^{}]*\}|[^\n]/g, (m) => (m.length > 1 ? m : ' '))
        : body;
      out += src[i++];
...
// Every package.json in the tree, mapped name -> directory, so a bare
// cross-package specifier in a workspace monorepo resolves to real source
// instead of silently producing no edge at all.
const workspacePkgs = new Map();
for (const f of tracked) {
  if (path.posix.basename(f) !== 'package.json' || SKIP_DIR.test(f)) continue;
  const j = readJsonAt(f);
  if (!j || typeof j.name !== 'string') continue;
  const dir = path.posix.dirname(f) === '.' ? '' : path.posix.dirname(f);
  workspacePkgs.set(j.name, { dir, entry: j.source || j.module || j.main || 'index.js' });
}
...
function resolveSpec(fromFile, spec) {
  ...
  // Workspace package: `@acme/b` or `@acme/b/sub`.
  for (const [name, pkg] of workspacePkgs) {
    if (spec !== name && !spec.startsWith(name + '/')) continue;
    const rest = spec.slice(name.length).replace(/^\//, '');
    const hit = rest
      ? resolveTo(path.posix.join(pkg.dir, rest)) || resolveTo(path.posix.join(pkg.dir, 'src', rest))
      : resolveTo(path.posix.join(pkg.dir, pkg.entry.replace(/^\.\//, '')))
        || resolveTo(path.posix.join(pkg.dir, 'src/index'))
        || resolveTo(path.posix.join(pkg.dir, 'index'));
    if (hit) return hit;
  }
...
for (const f of [...tracked]) {
  ...
  if (base === 'package.json') {
  ...
  } else if (/^requirements.*\.(txt|in)$/.test(base) || /(^|\/)requirements\/[^/]+\.(txt|in)$/.test(f)
             || base === 'pyproject.toml' || base === 'Pipfile' || base === 'setup.cfg'
    ...
    try {
      ...
      const isReqTxt = !['pyproject.toml', 'Pipfile', 'setup.cfg'].includes(base);
      const eat = (v) => {
      ...
      for (const raw of txt.split('\n')) {
        ...
        if (isReqTxt) {
          // `-r base.txt` / `-c constraints.txt` point at another manifest.
          // Guessing filenames could not find them; following the include does.
          const inc = line.match(/^(?:-r|--requirement|-c|--constraint)[=\s]+(\S+)/);
          if (inc) {
            const target = path.posix.normalize(path.posix.join(path.posix.dirname(f), inc[1]));
            if (!seenManifests.has(target)) { seenManifests.add(target); pendingManifests.push(target); }
            continue;
          }
          if (/^-/.test(line)) continue;      // -e ., --index-url, --hash
          eat(line);
          continue;
        }
        if (/^\[/.test(line)) {
...
// Follow `-r` / `-c` includes to whatever depth they nest.
for (let guard = 0; pendingManifests.length && guard < 50; guard++) {
  const rel = pendingManifests.shift();
  let txt;
  ...
  for (const raw of txt.split('\n')) {
    ...
    const inc = line.match(/^(?:-r|--requirement|-c|--constraint)[=\s]+(\S+)/);
    if (inc) {
      const target = path.posix.normalize(path.posix.join(path.posix.dirname(rel), inc[1]));
      if (!seenManifests.has(target)) { seenManifests.add(target); pendingManifests.push(target); }
      continue;
    }
    if (/^-/.test(line)) continue;
    const m2 = line.match(/^["']?([A-Za-z0-9_.-]+)/);
    if (m2 && m2[1]) declaredDeps.add(m2[1].toLowerCase().replace(/-/g, '_'));
  }
}
...
// `declaredDeps` has TWO jobs and they must not be conflated:
//   1. stop an edge being INVENTED (import stripe must not hit routes/stripe.py)
//   2. keep third-party noise out of the health counter
// Using it for both meant that declaring an in-repo package as a dependency —
// which src-layout and monorepo projects do routinely — turned a 100% broken
// graph into "0 unresolved, no warning". A name that names a real DIRECTORY in
// this repo is first-party for counting purposes no matter what a manifest says.
const repoDirs = new Set();
for (const f of tracked) {
  const parts = f.split('/');
  for (let i = 0; i < parts.length - 1; i++) repoDirs.add(parts[i]);
}
const looksFirstParty = (spec) => {
  if (spec.startsWith('.')) return true;
  const head = spec.replace(/^@/, '').split(/[./]/)[0];
  if (!head) return false;
  // A directory of this name exists here: always count it, even if declared.
  if (repoDirs.has(head)) return true;
  // A workspace package name is always first-party.
  for (const name of workspacePkgs.keys()) if (spec === name || spec.startsWith(name + '/')) return true;
