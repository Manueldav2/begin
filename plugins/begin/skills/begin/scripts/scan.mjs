...
function looksGenerated(file, src, lineCount) {
  if (/\.min\.(js|css)$/.test(file)) return 'minified';
  if (/(^|\/)(vendor|vendored|polyfills?|third[_-]?party|generated|__generated__)\//i.test(file)) return 'vendored';
  if (src.length / Math.max(1, lineCount) > 300) return 'minified';
  if (/^\s*(\/[/*]|#)\s*(@generated|AUTO-GENERATED|Code generated by|DO NOT EDIT)/im.test(src.slice(0, 2000))) return 'generated';
  return null;
}
...
for (const rec of info.values()) {
  ...
  if (ext === '.py') {
    ...
    while ((m = PY_FROM.exec(rec.nocomment))) {
      ...
      unresolvedBump(modHit || level > 0, mod || '.'.repeat(level));
    }
...
const record = (f) => {
  ...
  return {
    ...
    importance: +(rank.get(f) || 0).toFixed(6),
...
const out = {
  ...
  unparsed: unparsed.slice(0, 40).map((f) => ({ file: f, commits: churn.get(f) || 0, reason: info.get(f)?.skipped || 'no-code-lines' })),
};
...
// failure, and it looks exactly like "this code is dead" in the output.
{
  ...
  for (const [ext, b] of byLang) {
    if (b.n >= 20 && b.edges / b.n < 0.1 && ['.py', '.ts', '.tsx', '.js', '.jsx'].includes(ext))
...
md.push('> `complexityProxy` = decision-point count (not a parse). `maxNesting` = indentation depth. `importance` = PageRank over the import graph: high means many things depend on it, directly or transitively. Treat all three as leads to verify, not verdicts.');
md.push('');
...
for (const f of ranked.slice(0, TOP)) {
  ...
  shown.add(`${via}->${f}`);
  md.push(`- \`${via}\` (${surfaces.get(via)}) → …${r.hopsFromSurface} hop${r.hopsFromSurface > 1 ? 's' : ''}… → \`${f}\``);
}
